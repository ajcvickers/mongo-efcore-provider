...
public class NorthwindBulkUpdatesMongoTest : NorthwindBulkUpdatesTestBase<NorthwindBulkUpdatesMongoFixture<NoopModelCustomizer>>
{
    ...
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; set operations unsupported EF-X022
    ...
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; GroupBy unsupported EF-X022
...
#pragma warning restore xUnit1003
    ...
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; SelectMany unsupported EF-X022
    ...
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; cross-document navigation predicates unsupported EF-X022
    ...
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; non-entity projection sources unsupported EF-X022
...
#if EF10
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; joins / correlated subqueries unsupported EF-X022
...
#pragma warning restore xUnit1003
    ...
    // Fails: ExecuteUpdate/ExecuteDelete source restricted to a Where predicate; multiple-collection updates unsupported EF-X022
...
#if EF10
    // Fails: Throws a non-translation exception, but still throws EF-X002
    // The bulk source is still rejected, but since EF-117 (cross-collection Include/join) the read path lowers this
    // join far enough that the BulkUpdatesAsserter's pre-update snapshot read reaches the driver, which throws
    // ExpressionNotSupportedException before the provider's own bulk rejection — same artifact as Update_Concat/Union.
    public override Task Update_with_LeftJoin(bool async)
        => Assert.ThrowsAnyAsync<Exception>(() => base.Update_with_LeftJoin(async));
#endif

#if EF10
    // Fails: Throws a non-translation exception, but still throws EF-X002 (see Update_with_LeftJoin)

...
public class MongoPipelineFactoryTests
{
    ...
    public void Build_rewrites_limit_zero_constant_to_an_always_false_match()
    {
        ...
        Assert.Single(result);
        Assert.Equal(BsonDocument.Parse("{ $match: { _id: { $type: -1 } } }"), result[0]);
    }
    ...
    public void Build_rewrites_limit_zero_parameter_to_an_always_false_match()
    {
        ...
        var result = factory.Build(new Dictionary<string, object?> { ["take_count"] = 0 });
    ...
    public void Operand_limit_zero_is_rewritten_to_an_always_false_match_inside_Build()
    {
        ...
        var innerPipeline = result[0]["$unionWith"]["pipeline"].AsBsonArray;
        Assert.Equal(BsonDocument.Parse("{ $match: { _id: { $type: -1 } } }"), innerPipeline[0].AsBsonDocument);
    }

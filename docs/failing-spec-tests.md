...
| [EF-254](https://jira.mongodb.org/browse/EF-254) | _(no remaining `// Fails:` tags)_ | `.Skip(0).Take(0)` with a parameter now produces the expected empty result on both the driver-LINQ bridge (`Take(0)` → `Where(false)`) and the native path (a `$limit: 0` stage is rewritten to an always-false `$match` — see `MongoPipelineFactory.NormalizePagingStages`). EF-254 no longer has any active `// Fails:` tags. | 0 |

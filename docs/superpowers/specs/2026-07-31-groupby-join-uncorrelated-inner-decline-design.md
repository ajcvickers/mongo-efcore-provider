...
| Spec-level | `Join_complex_GroupBy_Aggregate`, `GroupJoin_complex_GroupBy_Aggregate`, `Join_GroupBy_Aggregate_in_subquery` + the 6 retargeted Join tests | the guard (they go back to executing and returning wrong data, which `AssertNativeTranslationFailedAsync` rejects) |
